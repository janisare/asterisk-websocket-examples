...
class ast_ws_server(AstAriWebSocketServer):
    def __init__(
        ...
        super().__init__(
            ari_host, ari_port, ari_credentials, ari_protocol, tag, log_level
        )
        ...
        self.mws = AstMediaWebSocketServer(
            self.media_host,
            self.media_port,
            self.media_credentials,
            self.media_protocol,
            tag=tag,
            log_level=log_level,
        )
    ...
    async def handle_dial(self, msg):
        ...
        if msg["dialstatus"] == "ANSWER":
            ...
            await self.send_request(
                "POST", f"bridges/{sess.bridge_id}/addChannel?channel={sess.ws_channel}"
            )
        ...
        if "websocket" in msg["channel"]["dialplan"]["app_data"]:
            ...
            if sess is not None:
                if sess.incoming_channel is not None:
                    ...
                    await self.send_request(
                        "DELETE", f"channels/{sess.incoming_channel}"
                    )
...
async def main(args):
    ...
    event_handler = ast_ws_server(
        args.ari_bind_address,
        args.ari_bind_port,
        ari_creds,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--ari-bind-address",
        type=str,
        help="Address to bind ARI websocket server to. Default=localhost",
        default="localhost",
        required=False,
    )
    parser.add_argument(
        ...
        "--ari-bind-port",
        type=str,
        help="Port bind ARI websocket server to",
        required=True,
    )
    parser.add_argument(
        "-awp",
        "--ari-websocket-protocol",
        type=str,
        help="Protocol for ARI websocket. Default=ari",
        required=False,
        default="ari",
    )
    parser.add_argument(
        ...
        "--ari-user",
        type=str,
        help="ARI user to authenticate incoming connections against",
        required=False,
    )
    parser.add_argument(
        "-aP", "--ari-password", type=str, help="Password for ARI user", required=False
    )
    parser.add_argument(
        "-ma",
        "--media-bind-address",
        type=str,
        help="Address to bind Media websocket server to. Default=localhost",
        default="localhost",
        required=False,
    )
    parser.add_argument(
        ...
        "--media-bind-port",
        type=str,
        help="Port to bind media websocket to",
        required=True,
    )
    parser.add_argument(
        "-mwp",
        "--media-websocket-protocol",
        type=str,
        help="Protocol for Media websocket. Default=media",
        required=False,
        default="media",
    )
    parser.add_argument(
        ...
        "--media-websocket-id",
        type=str,
        help="Connection name from websocket_client.conf. Default=media_connection1",
        required=False,
        default="media_connection1",
    )
    parser.add_argument(
        "-mU",
        "--media-user",
