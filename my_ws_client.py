...
class WSClient(AstAriWebSocketClient):
    ...
    async def handle_stasisstart(self, msg):
        ...
        if "incoming" in msg['channel']['dialplan']['app_data']:
            ...
            await self.send_request("POST", f"channels/{sess.other_channel}/dial?caller={incoming_id}&timeout=5")
        ...
        if "incoming" in msg['channel']['dialplan']['app_data']:
            ...
            if sess is not None:
                if sess.other_channel is not None:
                    ...
                    await self.send_request("DELETE", "channels/%s" % sess.other_channel)
...
async def main(args):
    event_handler = WSClient(args.ari_host, args.ari_port, args.stasis_app,
...
if __name__ == "__main__":
    description = (
        'Command line utility to test ARI client connections'
    )
